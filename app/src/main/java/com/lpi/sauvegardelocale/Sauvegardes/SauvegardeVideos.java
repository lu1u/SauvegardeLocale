...
import com.lpi.sauvegardelocale.Sauvegardes.objets.SavedVideo;
import com.lpi.sauvegardelocale.utils.FileUtils;
import com.lpi.sauvegardelocale.utils.Preferences;

public class SauvegardeVideos extends Sauvegarde
{
	public static String REPERTOIRE = "Videos";
	...
	protected STATUS executeSauvegarde(@NonNull SauvegardeAsynchrone t)
	{
		...
		REPERTOIRE = Preferences.getInstance(_context).getString(Preferences.PREF_REPERTOIRE_VIDEOS, REPERTOIRE);
		try
		{
			...
			Cursor cursor = SavedVideo.getCursor(_context);
			if (cursor == null)
			...
			while (cursor.moveToNext() && !_annule)
			{
				...
				SavedVideo objet = new SavedVideo(cursor, _context);
				Log.d(TAG, "Video " + objet.Nom(_context));
	...
	public SauvegardeVideos(@NonNull final Context c, @NonNull InterfaceSauvegarde interfce, @NonNull final SauvegardeListener listener)
	{
	...
	public static void Compte(final Context context, final CompteObjetsListener listener)
	{
		new AsyncTask<Void, Void, Integer>()
		{
			...
			{
				Cursor cursor = SavedVideo.getCursor(context);
				if (cursor == null)
					return 0;
